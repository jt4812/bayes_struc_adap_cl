...
class BayesAdaptiveCNN(nn.Module):
    def __init__(self, args, exp_dict=None):
        ...
        assert self.channel_scaler in [0.125, 0.25, 0.5, 1]


        self.conv_mask_type = args["conv_mask_type"]

        self.n_conv_layers = 24 if "n_conv_layers" not in args else args["n_conv_layers"]
        self.depth_inference = args["depth_inference"]  # "z_threshold", "usage_threshold"
        self.usage_threshold = args.get("usage_threshold", 0.025)
        self.reestimate_beta_posterior = args.get("reestimate_beta_posterior", False)

        self.use_fc_layers = False
        self.build_model()
    ...
    def build_model(self):
        ...
        for idx in range(self.n_conv_layers - len(conv_layers)):
            conv_layers.append(
                BayesConvBlock(int(512 * self.channel_scaler), int(512 * self.channel_scaler), (2, 2), (1, 1), "same",
                               self.posterior_std,
                               has_norm=self.has_norm, residual=True, has_max_pool=False,
                               use_task_specific_norms=self.use_task_specific_norms, n_tasks=self.n_tasks)
            )
        ...
        if self.has_bayesian_head:
            ...
            self.out_layer = nn.ModuleList([
                nn.Linear(fc_width,
                          self.out_feature_dim if isinstance(self.out_feature_dim, int) else self.out_feature_dim[idx])
    ...
    def reset_for_new_task(self, task_id=None):
        ...
        if self.infer_model_structure:
            if self.reestimate_beta_posterior:
                task_mask = self.task_mask_list[task_id]
                model_usage = torch.tensor([(Z_l.sum() / Z_l.numel()).item() for Z_l in task_mask])
                self.structure_sampler.reset_for_new_task(model_usage)
            else:
    ...
    def generate_single_mask(self, n_samples=None):
        ...
        model_usage = torch.tensor([(Z_l.sum() / Z_l.numel()).item() for Z_l in Z_single])

        if self.depth_inference == "usage_threshold":
            threshold_new = max(3, (model_usage > self.usage_threshold).sum().item())
            for Z_l in Z_single[threshold_new:]:
                Z_l.data.zero_()
